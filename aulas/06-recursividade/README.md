...
**Data:** 14/09  
**Unidade:** 1  
**Pré-requisito:** aula 05 (pilha de chamadas)

## Objetivos

- Identificar caso-base e caso recursivo.
- Relacionar recursão à **pilha de execução** vista no depurador.
...
1. Reabra o depurador: chame `fatorial(4)` com breakpoint em `return n * fatorial(n - 1)` e **step into**. A call stack cresce.
2. Desenhe os quadros na lousa: `fat(4)` espera `fat(3)`, … até `fat(1) = 1`, depois os retornos multiplicam.
...
No depurador: step into até o caso-base; step out / resume vendo os retornos.
...
```bash
javac aulas/06-recursividade/src/*.java
java -cp aulas/06-recursividade/src DemoRecursao
```
...
A Unidade 1 cobra: revisão, classes, encapsulamento/relacionamentos, leitura de stack trace / debug conceitual, e recursão (escrever e rastrear). Instrumento em `aulas/07-prova-unidade-1`.
