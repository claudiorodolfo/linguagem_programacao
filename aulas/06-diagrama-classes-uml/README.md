# Aula 06 — Diagrama de classes UML
...
Generalização (`extends`) aparece como **spoiler de uma linha**: o triângulo vazio volta na aula 09. Interfaces (`«interface»`) voltam na aula 11.

## Roteiro

1. Reabra o desenho da aula 05 (caixas `Usuario`, `Emprestimo`, `Livro`, `Endereco`). Diga: “hoje isso vira UML de verdade, a língua em que vocês vão documentar o projeto o resto da graduação.”
...
A prova da Unidade 1 é na **semana seguinte** (28/09) e cobra este diagrama.
...
Nesta disciplina ele é o mapa da biblioteca e da conta — o mesmo vocabulário da aula 05, com uma grafia padronizada.
...
Visibilidade (bate com a aula 04):
...
| Ideia (aula 05) | Traço UML | Lembrete |
| --- | --- | --- |
...
| `0..*` ou `*` | zero ou mais |
| `1..*` | um ou mais |
| `0..5` | no máximo cinco (o `MAX_EMPRESTIMOS` da aula 05) |
...
`Emprestimo` como classe no meio do traço Usuario–Livro (classe associativa) é a forma “de livro”; a implementação da aula 05 (objeto `Emprestimo` com duas referências) é a mesma ideia.
...
Navegabilidade: se a seta aponta só para `Livro`, `Emprestimo` tem o campo; `Livro` **não** precisa da lista de empréstimos. A aula 05 fez exatamente isso.
...
- Generalização (triângulo vazio, `extends`) — aula 09.
- `«interface»` e `«abstract»` — aula 11.
...
## Ponte para a prova (28/09)

A Unidade 1 cobra as aulas 01 a 06: revisão, classes, atributos/métodos, encapsulamento/pacotes, relacionamentos **e** o diagrama de classes (ler, completar, traduzir para Java). Instrumento em `aulas/07-prova-unidade-1`.
