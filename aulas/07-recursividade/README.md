...
- Relacionar recursão à **pilha de execução** (quadros de chamada). A aula 13 (depuração) volta a essa pilha no depurador.
...
2. Se a IDE estiver à mão, chame `fatorial(4)` com breakpoint em `return n * fatorial(n - 1)` e **step into**. A call stack cresce — a aula 13 sistematiza o depurador.
3. Live coding: palíndromo e busca binária.
4. Mostre Fibonacci recursivo vs. iterativo (tempo).
5. Laboratório.

Abre a Unidade 2. Herança entra na aula 08 (05/10).
...
No depurador (opcional hoje; obrigatório na aula 13): step into até o caso-base; step out / resume vendo os retornos.
...
```bash
javac aulas/07-recursividade/src/*.java
java -cp aulas/07-recursividade/src DemoRecursao
```
